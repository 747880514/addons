...
class Index_huasuan_EweiShopV2Page extends WebPage
{
	public function main($goodsfrom = 'sale')
	{
		...
		if (!(empty($total))) {
			$sql = 'SELECT g.* FROM '
			       . tablename('ewei_shop_goods') . 'g'
			       . $sqlcondition . $condition . $groupcondition
			       // . ' ORDER BY g.`status` DESC, g.`displayorder` DESC, g.`id` DESC '
			       . ' ORDER BY g.`displayorder` DESC, g.`id` DESC '
	...
	public function change()
	{
		...
		if (!(in_array($type, array('title', 'marketprice', 'total', 'goodssn', 'productsn', 'displayorder', 'dowpayment','costprice','salesreal','sales')))) {
			show_json(0, array('message' => '参数错误'));
		}
