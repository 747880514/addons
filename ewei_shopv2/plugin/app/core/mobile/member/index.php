...
class Index_EweiShopV2Page extends AppMobilePage
{
    public function main()
    {
		...
		if ($merch_plugin && $merch_data['is_openmerch']) {
			...
			if($merch_data['apply_openmobile']){
				$hasmerch = true;
				$ismerch = pdo_fetchcolumn('select status from ' . tablename('ewei_shop_merch_user') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']));
				$merchtext = $ismerch>=1?'您已成为商家':'申请入驻成为商家';
			}
		}
		 else {
        ...
        $result['bind_agentid'] = $member["agentid"] == NULL ? 1 : 0;   //1 需要绑定 0 不需要绑定

        //查找推荐人
        $agent_sql = "SELECT nickname,weixin FROM ".tablename('ewei_shop_member')." WHERE id = :id";
        $agent_data = pdo_fetch($agent_sql, array(':id'=>$result['agentid']));
        $result['agent_nickname'] = $agent_data['nickname'];
        $result['agent_weixin'] = $agent_data['weixin'];

        //是否是花蒜团队长
        $result['isheads_huasuan'] = ($member['isheads_huasuan'] == 1 && $member['headsstatus_huasuan'] == 1) ? 1 : 0;

        //如果邀请码为空，设置邀请码
        if(empty($member['invitation_code']))
        {
            $invitation_code = str_pad($member["id"], 6, "0", STR_PAD_LEFT);
            $member['invitation_code'] = $invitation_code;
            pdo_update("ewei_shop_member", array('invitation_code'=>$invitation_code), array( "id" => $member["id"], "uniacid" => $member["uniacid"] ));
        }
        $result['invitation_code'] = $member['invitation_code'];

        //优惠券总数
        $sql = "select count(d.id) from " . tablename("ewei_shop_coupon_data") . " d";
        $sql .= " left join " . tablename("ewei_shop_coupon") . " c on d.couponid = c.id";
        ...
        $coupon_total = pdo_fetchcolumn($sql, array( ":openid" => $_W["openid"], ":uniacid" => $_W["uniacid"] ));
        $result['coupon_total'] = max($coupon_total, 0);
		...
		$result['hasmerch'] = $hasmerch;
		$result['merchtext'] = $merchtext;
