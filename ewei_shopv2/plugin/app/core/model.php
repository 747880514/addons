    ...
    /**
		...
		public function getPage($id = 0, $mobile = false) 
		{
			...
			if (!(empty($page))) 
			{
				...
				//下架验证
				foreach ($page['data']['items'] as $key => &$value) {
					foreach ($value['data'] as $k => &$v) {
						if($v['gid'] > 0)
						{
							$status = pdo_fetchcolumn("SELECT status FROM ".tablename("ewei_shop_goods")." WHERE id = :gid LIMIT 1", array(":gid"=>$v['gid']));
							if($status == 0)
							{
								unset($value['data'][$k]);
							}
		...
		public function goodsData($item, $mobile = false) 
		{
			...
			if ($item['id'] == 'goods') 
			{
				...
				else if (($item['params']['goodsdata'] == 2) && !(empty($item['params']['groupid'])) && $mobile) 
				{
                    ...
                    $baili_goodsid_arr = explode(',', $goodsids);
					...
					if (!(empty($group)) && !(empty($group['goodsids']))) 
					{
                        ...
                        foreach ($goods as $key => $value) {
                            $k = array_search($value['id'], $baili_goodsid_arr);
                            $baili_list[$k] = $value;
                        }
                        ksort($baili_list);
                        $goods = $baili_list;
