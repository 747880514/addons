...
class Room_EweiShopV2Page extends SeckillWebPage
{
	...
	public function queryoption()
	{
		...
		$goods = pdo_fetch('select id,title,marketprice,hasoption,nocommission,costprice from ' . tablename('ewei_shop_goods') . ' where uniacid = :uniacid and id = :goodsid ', $params);

		if (!empty($pid)) {
			$packgoods = pdo_fetch('select id,title,packageprice,commission1,commission2,commission3,`option`,goodsid,costprice from ' . tablename('ewei_shop_package_goods') . "\r\n                        where pid = " . $pid . ' and uniacid = :uniacid and goodsid = :goodsid ', $params);
		}
		else {
			$packgoods = array('title' => $goods['title'], 'marketprice' => $goods['marketprice'], 'costprice'=>$goods['costprice'], 'packageprice' => 0, 'commission1' => 0, 'commission2' => 0, 'commission3' => 0);
		}

		if ($goods['hasoption']) {
			$hasoption = 1;
			$option = array();
			$option = pdo_fetchall('SELECT id,title,marketprice,specs,displayorder,costprice FROM ' . tablename('ewei_shop_goods_option') . "\r\n            WHERE uniacid = :uniacid and goodsid = :goodsid  ORDER BY displayorder DESC,id DESC ", $params);
			...
			foreach ($option as $key => $value) {
				foreach ($package_option as $k => $val) {
					if ($value['id'] == $val['optionid']) {
						...
						$option[$key]['costprice'] = $value['costprice'];
						continue;
					}
				}
