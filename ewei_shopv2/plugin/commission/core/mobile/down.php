...
class Down_EweiShopV2Page extends CommissionMobileLoginPage 
{
	...
	public function get_list() 
	{
		...
		$list = pdo_fetchall('select * from ' . tablename('ewei_shop_member') . ' where uniacid = ' . $_W['uniacid'] . ' ' . $condition . '  ORDER BY isagent DESC, childtime DESC limit ' . (($pindex - 1) * $psize) . ',' . $psize);
		foreach ($list as &$row ) 
		{
